<template>
    ...
    <form>
        ...
        <input v-model="obj.category" type="text" list="income_category" placeholder="Выберите категорию">

        <datalist id="income_category">
    ...
    <div v-for="card in sortedAndSearchedCard" :key="card.id">
        <div class="card" v-if="card.type === 'income'" >
            ...
            <div class="card__option">
                <my-button class="btn--small" @click="removeCard(card); updateIncome()">X</my-button>
            </div>
        </div>
    </div>
</template>
...
export default {
  components: { MyButton, MyInput, MySelect },
    ...
    methods: {
        ...
        ...mapActions( {
            updateIncome: 'card/updateIncome',
        }),
